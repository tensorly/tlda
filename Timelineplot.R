...
tl.data <- readr::read_csv("data/TimeLine_Data.csv")

status_levels <- c("Social", "Political", "Celebrity","Countermovement")
status_colors <- c("#0070C0", "#228B22","#E4B04A", "#C00000")
...
timeline_plot<-timeline_plot+scale_color_manual(values=status_colors, labels=status_levels, drop = FALSE)+scale_fill_manual(values=status_colors, labels=status_levels, drop = FALSE)
...
# Show year text
timeline_plot<-timeline_plot+geom_label(data=year_tl.data, aes(x=year_date_range,y=-1,label=year_format, fontface="bold"),size=4,color='black')
# Show text for each milestone
timeline_plot<-timeline_plot+geom_label(data = tl.data %>%filter(text_position>=0), aes(y=text_position,label=`Most Prominent Topic`),size=9,show.legend = FALSE) 
timeline_plot<-timeline_plot+geom_label(data = tl.data %>%filter(text_position<=0), aes(y=text_position,label=`Most Prominent Topic`,size=Magnitude),show.legend = FALSE) +  scale_size(range = c(4.5, 8))


plot(timeline_plot)


pdf("plots/prom_topics.pdf",width = 18.5,height = 8.5)
timeline_plot
dev.off()